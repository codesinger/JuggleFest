#ifndef juggler_circuit_set_iterator_h_included
#define juggler_circuit_set_iterator_h_included 1

/*!
 * \file juggler_circuit_set_iterator.h
 *
 * \brief Contains the definition of juggler_circuit_set_iterator
 *
...
/*!
 * \brief An iterator over juggler_circuit_set
 */
...
public:
   ...
   */
  explicit juggler_circuit_set_iterator(
    juggler_circuit_set    &jcs)   /*!< Set over which we iterate           */
  :
  _jcs(jcs),
  ...
  /*!
   * \brief Delete the juggeler_circuit on which the iterator is positioned
   */
  void remove_current()
  {
    assert( !(_last == 0) );
    std::set<juggler_circuit_reference>::iterator  iter = _last_it;
    _jcs._jcs_set.erase(iter);
  }
  ...
  friend std::ostream &operator<<(
    ...
    const juggler_circuit_set_iterator  &cn)/*!< The object to be streamed  */
  {
...
private:
   ...
   */
  juggler_circuit_set_iterator(
    const juggler_circuit_set_iterator   &rhs);
   ...
   */
  juggler_circuit_set_iterator &operator=(
    const juggler_circuit_set_iterator   &rhs);
  ...
  const
  {
    os << "juggler_circuit_set_iterator";

    return os;
  }

  //! The set over which we iterate
  juggler_circuit_set                 &_jcs;
...
#endif                             /* juggler_circuit_set_iterator_h_included*/
